...
**MiniSearchEntry**:
可被微信搜索收录的 MiniApp 页面：首页、DiscoverPage，以及带稳定 ID 的 SubjectDetailPage、CharacterDetailPage 和 PersonDetailPage；不包括个人页或带搜索词、筛选条件等瞬态状态的页面。详情页以实体名加实体类型和 Bangumi X 作为页面及分享标题，页面内自定义导航栏保持简洁。详情入口以携带稳定 ID 的 `<navigator>` URL 供搜索爬虫发现。检索文本仅复用可核验的上游事实，不添加生成式推荐或解读。
