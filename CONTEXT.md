...
基于番组计划（Bangumi）公开数据的浏览前端。SSR server 部署在国内，通过 Cloudflare Worker 翻墙访问 bgm 上游 API。现有内容始终允许公开浏览，登录只为访客建立可选身份。
...
**Visitor**:
访问 Bangumi X 的人，无论是否登录，都可以浏览全部公开内容。
_Avoid_: User（无法表达是否已有身份）

**SignedInViewer**:
已通过统一账户建立身份的 Visitor。它是站点访问者身份，不是 Bangumi 数据中的 Person。
_Avoid_: Person, Bangumi User
...
**DetailRoute**:
MiniApp 跳转详情页的稳定 URL 约定：`/pages/{type}/{type}?id={id}`，其中 `type` 为 subject、character 或 person。由 `<navigator>` 生成，使搜索爬虫能发现并直接打开详情页。区别于 web 的浏览器导航。
_Avoid_: 把它当作独立领域概念--仅是 MiniApp 的路由/搜索发现约定。
...
**WebThemePreference**:
当前浏览器持有的 Web 端主题偏好，可选 light、dark 或 system；它不随 SignedInViewer 账户同步。与 MiniApp 的 ThemeMode 不同，Web 允许手动选择并记住偏好。
_Avoid_: 与 MiniApp ThemeMode 混用

**LayoutMode**:
MiniApp 的布局模式，值为 compact（紧凑，窄窗口）或 expanded（宽松，宽窗口）。与 ThemeMode 正交--ThemeMode 是主题维度（light/dark），LayoutMode 是布局维度。由当前窗口的可用宽度判定，不依赖设备类型；通过 `app.ts` globalData 存储冷启值，页面在窗口尺寸变化时重新计算并应用；`utils/layout.ts` 提供统一的计算与应用入口。Skyline 不支持 CSS `@media`，故采用运行时判断而非声明式断点。
...
expanded 由当前窗口的可用宽度驱动，不依赖设备类型；覆盖平板横屏与竖屏、分屏窗口和折叠屏展开态。窗口尺寸变化时重新计算并应用 LayoutMode。同一功能与导航模型在两种模式间保持稳定，但 expanded 可以采用专门的信息编排，而不只是增加列数和留白。
_Avoid_: 仅根据 `deviceType` 推断布局--设备标签不能准确表达横屏、分屏或折叠状态下的实际可用空间。

**MiniSearchEntry**:
小程序产品名为“番迹”。可被微信搜索收录的 MiniApp 页面：首页、DiscoverPage，以及带稳定 ID 的 SubjectDetailPage、CharacterDetailPage 和 PersonDetailPage；不包括个人页或带搜索词、筛选条件等瞬态状态的页面。详情页以实体名加实体类型和“番迹”作为页面及分享标题，页面内自定义导航栏保持简洁。详情入口以携带稳定 ID 的 `<navigator>` URL 供搜索爬虫发现。检索文本仅复用可核验的上游事实，不添加生成式推荐或解读。
_Avoid_: 将其等同于 DiscoverPage 或任意搜索结果页
