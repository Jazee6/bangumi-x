...
import { searchPersons } from "@/server/functions.ts";
...
function PersonsPage() {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteQuery({
    queryKey: ["persons", "search", { keyword, limit: PAGE_SIZE }] as const,
    queryFn: async ({ pageParam = 0 }): Promise<PagedResponse<Person>> =>
      searchPersons({
  ...
  return (
    <article>
      ...
      <div className="mb-6">
        <SearchInput value={keyword} onSearch={handleSearch} placeholder="搜索人物..." />
      </div>
