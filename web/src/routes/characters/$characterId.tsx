...
export const Route = createFileRoute("/characters/$characterId")({
  ...
  head: ({ loaderData, params }) => {
    ...
    if (character.blood_type) facts.push(`血型 ${BloodTypeLabel[character.blood_type]}`);
      ...
      <header className="flex flex-col gap-6 sm:flex-row">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {character.type && (
              <Badge variant="secondary">{CharacterTypeLabel[character.type] ?? ""}</Badge>
            )}
            {character.gender && <Badge variant="outline">{character.gender}</Badge>}
            {character.blood_type && (
              <Badge variant="outline">血型 {BloodTypeLabel[character.blood_type]}</Badge>
            )}
      ...
      <section className="mt-8">
        <Tabs defaultValue="subjects">
          <TabsList>
            ...
            <TabsTrigger value="persons">相关人物 ({persons.length})</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="persons" className="mt-4">
            {persons.length === 0 ? (
              <p className="py-8 text-center text-muted-foreground">暂无相关人物</p>
            ) : (
